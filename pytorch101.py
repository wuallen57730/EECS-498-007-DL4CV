...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.arange(start,stop,1, dtype=float64)
    x = x[x%10==0]
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y = torch.zeros(len(x), 1+max(x), dtype=float32)  # y.shape(N, C)
    x_tensor = torch.tensor(x, dtype=torch.long)
    # scatter_(dim, index, value)
    # x_tensor.view(-1, 1) 將 x_tensor 轉為shape為(N, 1)的張量
    y.scatter_(1, x_tensor.view(-1, 1), 1)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y_inter = x.reshape(2,3,-1)   # shape(2, 3, 4)

    y_inter = y_inter.transpose(0, 1) # shape(3, 2, 4)

    y = y_inter.reshape(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    min_idxs = torch.argmin(x, dim=1)

    row_indices = torch.arange(x.shape[0])
    
    y[row_indices, min_idxs] = 0
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # 計算每列均值，形狀 (N,)
    mu = torch.mean(x, dim=0) # == torch.sum(x, dim=0) / M
    
    # 計算每列標準差：sqrt(sum((x - mu)**2, dim=0) / M)
    # Broadcasting：x - mu，mu 形狀 (N,) 擴展為 (M, N)
    squared_diff = (x - mu) ** 2
    print("\nsquared_diff:",squared_diff)
    print()
    sigma = torch.sqrt(torch.sum(squared_diff, dim=0) / M)
    print("\nsigma:",sigma)
    print()
    # 避免除以 0，將標準差為 0 的位置設為 1
    sigma = torch.where(sigma == 0, torch.tensor(1.0, dtype=x.dtype), sigma)
